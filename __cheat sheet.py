...
# (7) 데이터 분할
# 
#     1. 랜덤 분할 : train test 데이터세트를 나누어서 학습된 데이터를 검증할 수 있으며, 분할 시에 무작위로 사용자가 지정한 비율로 분할한다. 전체 분석 데이터 중 머신러닝 모델을 학습시키기 위한 학습용 데이터와 테스트용 데이터를 나누어서 적용시키는 이유는 모델 결과가 다른 데이터에도 적용 가능한 지, 일반화가 가능한 지를 검증하기 위함이다.
#     
#     2. 층화 추출 기법 : 종속변수가 범주형인 경우에는 종속변수의 클래스의 비율을 기준으로 학습용 데이터와 테스트용 데이터의 비율이 동일하게 분할한다. 즉, 클래스의 편항을 막을 수 있다.
#     
...
#     오버샘플링 / 언더샘플링
#     
#     1. 오버 샘플링 : 오버 샘플링 기법은 비중이 적은 데이터를 추가로 생성해 수를 늘려 데이터 불균형을 극복하는 방식이다. 소수 레이블을 가진 데이터 세트를 다수 레이블을 가진 데이터 세트의 수만큼 증식시켜 학습에 충분한 데이터를 확보하는 기법이다. 언더 샘플링은 데이터 손실의 문제로 인해 예측 성능이 저하되는 단점이 있으므로, 일반적으로는 불균형한 데이터를 처리하는 방식으로 오버 샘플링을 사용한다.
#     
...
#             - SMOTE는 Resampling 방식보다 속도가 느리다는 단점이 있지만, 데이터를 단순히 동일하게 증식시키는 방식이 아니기 때문에, 과적합 문제를 예방할 수 있다는 장점이 있다.
#             
#     2. 언더 샘플링 : Under sampling은 다수 클래스를 감소시켜 소수 클래스 개수에 맞추는 방식으로, 대표적으로 random으로 다수의 클래스의 데이터를 선택하여 삭제하는 RandomUnderSampler, 서로 다른 클래스가 있을 때, 서로 다른 클래스에 가장 가까운 데이터들이 토멕 링크로 묶여서 토멕 링크 중 다수 클래스의 데이터를 제거하는 Tomek Link 방식이 있다.
#     
#     * 보통의 경우 답안 : 둘 중 해당 데이터에서는 Oversampling이 적합하다. 데이터가 총 768개로 당뇨병 화자를 대표하기에는 너무 적은 data이다. 심지어 환자의 수는 768명 중 268명 뿐이다. Undersampling을 선택하게 되면 전체 데이터가 더 적어지기 때문에 오버피팅이 일어날 위험이 더 크므로 oversampling을 선택하겠다.
...
#     
#     해당 데이터에서는 종속변수의 값이 연속형 변수이므로 회귀분석이 적합하다. 회귀분석에서 다중공선성의 문제를 해결하는 것이 중요한데 svm은 커널트릭을 통해, xgboost와 randomforest는 트리 모델을 통해 다중공선성을 해결할 수 있다. 그러므로 회귀분석을 지원하는 위 3가지의 알고리즘은 연속형 변수를 예측하기에 적합하다.
# 
# 
...
# 
#     속도 개선을 위한 차원축소방법인 PCA를 사용할 수 있다. 예측의 성능은 원본의 데이터를 그대로 사용하는 것보다 떨어질 수 있지만, 차원을 축소함으로써 예측의 속도는 훨씬 상승시킬 수 있다. *PCA를 사용할 때, 데이터의 스케일에 따라 각 주성분이 설명가능한 분산량이 달라질 수 있기 때문에 데이터 스케일링을 꼭 해주어야 한다.
